...
type Category = 'Paper' | 'Cards' | 'Finishing' | 'Vinyl' | 'Ink';
...
const categories: Array<'All' | Category> = ['All', 'Paper', 'Cards', 'Finishing', 'Vinyl', 'Ink'];
...
function Dashboard() {
  ...
  const revealInventory = () => {
    setView('inventory');
    window.setTimeout(() => inventoryRef.current?.scrollIntoView({ behavior: 'smooth', block: 'start' }), 20);
  ...
  const adjustQuantity = (id: string, amount: number) => {
    setItems((current) => current.map((item) => item.id === id ? { ...item, quantity: Math.max(0, item.quantity + amount) } : item));
    setFlashId(id);
    window.setTimeout(() => setFlashId((current) => current === id ? null : current), 450);
    const item = items.find((entry) => entry.id === id);
    if (item) showToast(`${item.name} ${amount > 0 ? 'restocked' : 'adjusted'}.`);
  };
  ...
  return (
    <div className="inventory-shell paper-grain min-h-[100dvh] text-[#111522]">
      <div className="flex min-h-[100dvh]">
        <aside className={`fixed inset-y-0 left-0 z-40 flex w-[268px] flex-col border-r border-[#f0dfe2] bg-white px-5 py-6 text-[#111522] transition-transform duration-200 lg:relative lg:translate-x-0 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
          <div className="flex items-start justify-between">
            <button type="button" onClick={() => setView('overview')} className="group text-left" data-testid="button-brand-home">
          ...
          <div className="mt-14">
            ...
            <nav className="mt-3 space-y-1" aria-label="Main navigation">
              ...
              <button type="button" onClick={revealInventory} className={`flex w-full items-center gap-3 rounded-lg px-3 py-3 text-left text-sm font-semibold transition ${view === 'inventory' ? 'bg-[#fff0f2] text-[#e40012]' : 'text-[#747983] hover:bg-[#fff4f5] hover:text-[#111522]'}`} data-testid="button-nav-inventory">
              ...
              <button type="button" onClick={() => showToast('Cabinet map is ready for a future layout pass.', 'neutral')} className="flex w-full items-center gap-3 rounded-lg px-3 py-3 text-left text-sm font-semibold text-[#747983] transition hover:bg-[#fff4f5] hover:text-[#111522]" data-testid="button-nav-cabinet-map">
                <MapPin size={17} /> Cabinet map <span className="ml-auto font-mono text-[9px] uppercase tracking-wider text-[#aa7c82]">Soon</span>
              </button>
            </nav>
          </div>
        ...
        {sidebarOpen && <button type="button" aria-label="Close navigation" className="fixed inset-0 z-30 bg-[#1c2b45]/35 lg:hidden" onClick={() => setSidebarOpen(false)} data-testid="button-overlay-close" />}
        ...
        <main className="min-w-0 flex-1">
          <header className="flex h-[76px] items-center justify-between border-b border-[#f0dfe2] bg-white/95 px-5 backdrop-blur-md sm:px-8 lg:px-12">
            <div className="flex items-center gap-3">
              <button type="button" onClick={() => setSidebarOpen(true)} className="rounded-lg p-2 text-[#4e535f] hover:bg-[#fff0f2] lg:hidden" aria-label="Open menu" data-testid="button-open-menu"><Menu size={20} /></button>
            ...
            <div className="flex items-center gap-2 sm:gap-4">
              <button type="button" onClick={() => searchRef.current?.focus()} className="hidden items-center gap-2 rounded-lg px-2 py-1.5 text-xs text-[#747983] hover:bg-[#fff0f2] md:flex" data-testid="button-focus-search"><Search size={15} /> Find material <kbd className="rounded border border-[#f0d5d9] bg-[#fff7f8] px-1.5 py-0.5 font-mono text-[9px]">/</kbd></button>
              <div className="h-7 w-px bg-[#f0dfe2] hidden sm:block" />
              <div className="flex items-center gap-2">
                 <div className="flex h-8 w-8 items-center justify-center rounded-full bg-[#111522] text-xs font-extrabold text-white" data-testid="avatar-user">MC</div>
                 <div className="hidden text-left sm:block"><p className="text-xs font-bold">Mara Chen</p><p className="font-mono text-[9px] uppercase tracking-wider text-[#aa7c82]">Operations lead</p></div>
                 <ChevronDown size={14} className="hidden text-[#aa7c82] sm:block" />
              </div>
            </div>
          </header>

          <div className="mx-auto max-w-[1440px] px-5 py-8 sm:px-8 lg:px-12 lg:py-10">
            <section className="appear flex flex-col justify-between gap-6 sm:flex-row sm:items-end">
              <div>
              ...
              <button type="button" onClick={() => { setDialog({ mode: 'add' }); setView('inventory'); }} className="group inline-flex shrink-0 items-center justify-center gap-2 rounded-lg bg-[#e40012] px-4 py-3 text-sm font-bold text-white shadow-[4px_4px_0_#111522] transition hover:-translate-y-0.5 hover:shadow-[5px_6px_0_#111522] active:translate-y-0" data-testid="button-add-item">
            ...
            <section className="appear-2 mt-9 grid gap-4 sm:grid-cols-2 xl:grid-cols-4" aria-label="Inventory summary">
              ...
              <StatCard label="Cabinet bays" value={5} detail="organized locations" icon={<MapPin size={18} />} tone="gold" testId="stat-bays" />
            </section>
            ...
            <section className="appear-3 mt-8 grid gap-5 xl:grid-cols-[1.55fr_.85fr]">
              <div className="rounded-xl border border-[#f0dfe2] bg-white p-5 sm:p-6 soft-shadow">
                <div className="flex items-start justify-between gap-4">
                   ...
                   <button type="button" onClick={revealInventory} className="hidden items-center gap-1 text-xs font-bold text-[#e40012] hover:text-[#a9000d] sm:flex" data-testid="button-view-all-alerts">View inventory <ArrowUpRight size={14} /></button>
                </div>
               ...
               <div className="relative overflow-hidden rounded-xl bg-[#111522] p-5 sm:p-6">
                 <div className="absolute -right-9 -top-12 h-36 w-36 rounded-full border-[22px] border-[#e40012]/35" />
                 <div className="absolute bottom-[-34px] right-12 h-24 w-24 rounded-full border-[13px] border-[#f6a6ad]/25" />
                <div className="relative">
                   <p className="font-mono text-[10px] uppercase tracking-[0.18em] text-[#f6a6ad]">Next move</p>
                   <h2 className="mt-2 max-w-xs text-2xl font-extrabold leading-tight tracking-[-0.05em] text-white">Keep the next job moving.</h2>
                   <p className="mt-3 max-w-xs text-sm leading-5 text-[#e7cbd0]">A quick count now keeps the next maintenance or controls run from stalling.</p>
                   <button type="button" onClick={revealInventory} className="mt-6 inline-flex items-center gap-2 rounded-lg bg-[#e40012] px-3.5 py-2.5 text-xs font-bold text-white transition hover:bg-[#c80010]" data-testid="button-open-inventory">Open inventory <ArrowUpRight size={14} /></button>
                </div>
              </div>
            </section>

            <section ref={inventoryRef} className="appear-4 mt-10 scroll-mt-6" aria-labelledby="inventory-heading">
              <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
                 <div><p className="font-mono text-[10px] uppercase tracking-[0.18em] text-[#d10011]">The materials register</p><h2 id="inventory-heading" className="mt-1 text-2xl font-extrabold tracking-[-0.055em]">Inventory, at a glance.</h2></div>
                 <div className="flex items-center gap-2 text-xs text-[#777c86]"><span className="h-2 w-2 rounded-full bg-[#10a77b]" /> Saved automatically in this browser</div>
              </div>
                ...
                <div className="flex flex-col gap-3 md:flex-row">
                  <label className="relative flex-1">
                    ...
                    <Search size={16} className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-[#8993a0]" />
                     <input ref={searchRef} type="search" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search materials or cabinet locations" className="h-11 w-full rounded-lg border border-[#f0dfe2] bg-[#fff8f9] pl-10 pr-4 text-sm text-[#111522] placeholder:text-[#9ca2aa] focus:border-[#e40012] focus:bg-white focus:outline-none" data-testid="input-search-inventory" />
                  </label>
                   <button type="button" onClick={() => showToast('Inventory is already saved locally.', 'neutral')} className="hidden items-center justify-center gap-2 rounded-lg border border-[#f0dfe2] px-3 text-xs font-bold text-[#596579] hover:bg-[#fff0f2] md:flex" data-testid="button-filter-info"><SlidersHorizontal size={15} /> Saved view</button>
                </div>
                 ...
                 <div className="mt-4 overflow-hidden rounded-xl border border-[#f0dfe2] bg-white soft-shadow">
                  <div className="overflow-x-auto">
                    <table className="w-full min-w-[850px] border-collapse text-left">
                      ...
                      <tbody>{filteredItems.map((item) => <InventoryRow key={item.id} item={item} flash={flashId === item.id} onAdjust={adjustQuantity} onEdit={() => setDialog({ mode: 'edit', item })} onDelete={() => setDeleteTarget(item)} />)}</tbody>
                    </table>
                  </div>
...
function InventoryRow({ item, flash, onAdjust, onEdit, onDelete }: { item: InventoryItem; flash: boolean; onAdjust: (id: string, amount: number) => void; onEdit: () => void; onDelete: () => void }) {
  const meta = categoryMeta[item.category];
  const Icon = meta.icon;
